    ...
    public partial class FirstTimeLogin : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (Request.QueryString["username"] != null)
                {
                    ...
                    int uid = Global.GetUserIDByUsername(Request.QueryString["username"]);
                    string role = Global.GetRole(uid);

                    Session["tempRole"] = role;

                    if (role != "client user")
                    {
                        signature.Visible = false;
                        hdnSignatureData.Visible = false;
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (txtNewPassword.Text.CompareTo(txtConfirmPassword.Text) == 0)
            {
                ...
                {
                    role = Session["tempRole"].ToString();
                }


                if (role == "client user" || role == "cosec user")
                {
                    updateQuery = "UPDATE [dbo].[User] SET password = @password, graphicPic = @graphicPic, signaturePic = @signaturePic, status = @status WHERE username = @username";

                }
                else
                {
                    updateQuery = "UPDATE [dbo].[User] SET password = @password, status = @status WHERE username = @username";
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    using (SqlCommand command = new SqlCommand(updateQuery, connection))
                    {
                        ...
                        if (role == "client user" || role == "cosec user")
                        {
                            ...
                            string signerName = Global.GetNameByUsername(Request.QueryString["username"]);
                            byte[] digiCert = CreateSelfSignedCertificate(Request.QueryString["username"], signerName);

                            command.Parameters.AddWithValue("@graphicPic", graphSign);
                            command.Parameters.AddWithValue("@signaturePic", digiCert);
                        }

                        command.Parameters.AddWithValue("@password", HashPassword(txtConfirmPassword.Text));
            ...
            {
                string script = "alert('New password and confirm password are not the same.');";
        ...
        protected byte[] CreateSelfSignedCertificate(string subjectName, string name)
        {
            ...
            enroll.CertificateFriendlyName = name; // Optional: add a friendly name
