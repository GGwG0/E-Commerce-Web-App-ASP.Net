...
namespace FYP_WebApplication
{
    public partial class WebForm1 : System.Web.UI.Page
    {
        ...
        protected DataTable GetDataFromTable()
        {
            ...
            if (Session["currentRole"].ToString() == "client user" || Session["currentRole"].ToString() == "cosec user" || Session["currentRole"].ToString() == "service user")
            {
            ...
            else if (Session["currentRole"].ToString() == "cosec admin" || Session["currentRole"].ToString() == "client admin" || Session["currentRole"].ToString() == "service admin")
            {
                query = "SELECT A.*, U.username, C.comName FROM Audit A LEFT JOIN [User] U ON A.UserID = U.userID LEFT JOIN Company C on A.CompanyID =C.companyID  WHERE A.UserID = @userid OR A.CompanyID = @CompanyID ORDER BY date DESC;";
            }
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if (Session["currentRole"].ToString() == "client user" || Session["currentRole"].ToString() == "cosec user" || Session["currentRole"].ToString() == "service user")
            {
                query = "SELECT A.*, U.username, C.comName FROM Audit A LEFT JOIN [User] U ON A.UserID = U.userID LEFT JOIN Company C on A.CompanyID =C.companyID  WHERE A.UserID = @userid AND (action LIKE @searchTerm OR audit_ID LIKE @searchTerm OR req_ID LIKE @searchTerm  OR CONVERT(VARCHAR, date, 23) LIKE @searchTerm OR U.username LIKE @searchTerm OR C.comName LIKE @searchTerm) ORDER BY date DESC;";

            }
            else if (Session["currentRole"].ToString() == "cosec admin" || Session["currentRole"].ToString() == "client admin" || Session["currentRole"].ToString() == "service admin")
            {
                query = "SELECT A.*, U.username, C.comName FROM Audit A LEFT JOIN [User] U ON A.UserID = U.userID LEFT JOIN Company C on A.CompanyID =C.companyID  WHERE A.UserID = @userid OR A.CompanyID = @CompanyID AND (action LIKE @searchTerm OR audit_ID LIKE @searchTerm OR req_ID LIKE @searchTerm  OR CONVERT(VARCHAR, date, 23) LIKE @searchTerm OR U.username LIKE @searchTerm OR C.comName LIKE @searchTerm) ORDER BY date DESC;";
            }
