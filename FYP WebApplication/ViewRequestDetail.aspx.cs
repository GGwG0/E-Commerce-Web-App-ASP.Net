...
namespace FYP_WebApplication
{
    public partial class ViewRequestDetail : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (type != "Board Resolution Request")
                {
                    btnBR.Visible = false;
                ...
                if (Convert.ToInt32(Session["createdByID"]) != Convert.ToInt32(Session["userid"]))
                    {
                        btnGeneral.Visible = false;
                    }
            }
        ...
        public string GetRequestType(int requestID)
        {
            ...
            {
                string query = "SELECT [type] FROM [dbo].[Request] WHERE [requestID] = @RequestID";
                ...
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        // You may also throw an exception here based on your error handling strategy.
                    }
                }
            }
    ...
    private string GetRequestStatus(int requestID)
        ...
        private void BindComment(int requestID, int userid)
        {
            ...
            string connectionString = ConfigurationManager.ConnectionStrings["ConnectionString"].ConnectionString;
            string selectQuery = "Select C.*, U.profilePicture, U.username from Comment C left join [User] U ON C.userID = U.userID where requestID = @requestID" +
                " Order By writtenDate Asc;";
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                SqlCommand command = new SqlCommand(selectQuery, connection);
                command.Parameters.AddWithValue("@requestID", requestID);
        ...
        protected void btnSend_Click(object sender, ImageClickEventArgs e)
        {
            if (!string.IsNullOrEmpty(txtMessage.Text))
            {
                ...
                if (Session["userid"].ToString() == Session["createdByID"].ToString())
                {
        ...
        protected void sendEmail()
        {
            ...
            String to = Global.GetEmailByID(Convert.ToInt32(Session["userid"].ToString()));

            message.From = new MailAddress("awieling0777@gmail.com");
            message.To.Add(new MailAddress(to));
            message.Subject = "A new comment made for the request you created!";
            message.Body = "Dear sir/ madam,  " + "\n" + " Good day! Your company secretary has commented on the request with the ID ( " + requestID+ " ). Please follow up for more information on the website" 
       + "Thank you.";
        ...
        protected void DeleteRecord(int requestID, string status)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                if (success2 > 0)
                {
                    ...
                    command2.Parameters.AddWithValue("@to", Convert.ToInt32(Global.GetUserIDByUsername(lblAssignedTo.Text)));
        ...
        protected void btnGeneral_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                string query = "UPDATE [dbo].[Request] SET [status] = 'Completed' WHERE [requestID] = @RequestID";

                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    command.Parameters.AddWithValue("@RequestID",Convert.ToInt32(Request.QueryString["id"]));
            ...
            if (a > 0)
            {
                Global.AddNotification("Completed the request", Convert.ToInt32(Request.QueryString["id"]), Convert.ToInt32(Session["assignedTo"]), Convert.ToInt32(Session["createdByID"]));
            }
