...
namespace FYP_WebApplication
{
    public partial class SignBoardResolution : System.Web.UI.Page
    {
        ...
        private string GetUserGoogleAuthKey(int user_ID)
        {
            string connectionString = ConfigurationManager.ConnectionStrings["ConnectionString"].ConnectionString;
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                string sql = "SELECT googleAuthKey FROM [dbo].[User] WHERE userID = @userID";
        ...
        {
            string googleAUthekey = GetUserGoogleAuthKey(Convert.ToInt32(Session["userid"]));
            string verificationCode = otpTxt.Text.Trim(); 

            TwoFactorAuthenticator twoFactorAuthenticator = new TwoFactorAuthenticator();
            bool isValid = twoFactorAuthenticator.ValidateTwoFactorPIN(googleAUthekey, verificationCode, true);
            ...
            {
                //error page
                string script = "alert('Your OTP is incorrect.');";
           ...
           // SqlConnection connection = new SqlConnection(connectionString);
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                string query = @"select U.userID from[User] U
                                 LEFT JOIN[User] C ON C.companyID = U.companyID
                                 where C.userID = @id and U.position = 'BOD'";

                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    command.Parameters.AddWithValue("@id", GetCreatedBy(Convert.ToInt32(Request.QueryString["reqID"])));
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            int userID = Convert.ToInt32(reader["userID"].ToString());
                            AddNotification("Board resolution has been published and waiting for completion of signatures", Convert.ToInt32(Request.QueryString["reqID"]), userID, Convert.ToInt32(Session["userid"]));
        ...
        {
            string query = "SELECT U.username, U.graphicPic, U.signaturePic, U.userID, S.createdDate FROM Signature S LEFT JOIN [User] U ON S.userID = U.userID WHERE S.boardReID = @boardReID";
                ...
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            pdfBytes = ApplySignature(pdfBytes, cert, graphic, offsetXArray[userIndex], offsetYArray[userIndex], gWidth, dWidth, height, username, username, newPage,createdDate);
        ...
        protected byte[] ApplySignature(byte[] pdfBytes, byte[] certBytes, byte[] graphicSign, float offsetX, float offsetY, int gWidth, int dWidth, int height, string signatureFieldName, string userName, int targetPage, DateTime now)
        {
