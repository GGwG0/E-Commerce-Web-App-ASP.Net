    ...
    public partial class BoardResolutionDashboard : System.Web.UI.Page
    {
        ...
        protected DataTable GetDataFromTable()
        {
            ...
            if (Session["currentRole"].ToString() == "client user")
            {
                ...
                ddlStatus.DataBind();
                query = "Select * from BoardResolution B LEFT JOIN [User] U ON B.createdBy  = U.userID LEFT JOIN [User] M ON M.userID = @id WHERE U.companyID = M.companyID AND B.type <> 'T'";

            }
            else if (Session["currentRole"].ToString() == "cosec user")
            {
                query = "SELECT * FROM BoardResolution B LEFT JOIN Request R ON B.requestID = R.requestID WHERE B.type <> 'T' and R.assignedTo = @id;";
            }
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if (Session["currentRole"].ToString() == "client user")
            {
                ddlStatus.Items.Remove("Deleted");
                ddlStatus.DataBind();
                query = "Select * from BoardResolution B LEFT JOIN [User] U ON B.createdBy  = U.userID LEFT JOIN [User] M ON M.userID = @id WHERE U.companyID = M.companyID AND B.type <> 'T' and (title LIKE @searchTerm OR description LIKE @searchTerm OR CONVERT(VARCHAR, due_date, 23) LIKE @searchTerm OR CONVERT(VARCHAR, created_date, 23) LIKE @searchTerm OR requestID LIKE @searchTerm OR createdBy LIKE @searchTerm);";

                // query = "SELECT * FROM BoardResolution WHERE type <> 'T' and createdBy = @id and (title LIKE @searchTerm OR description LIKE @searchTerm OR CONVERT(VARCHAR, due_date, 23) LIKE @searchTerm OR CONVERT(VARCHAR, created_date, 23) LIKE @searchTerm OR requestID LIKE @searchTerm OR createdBy LIKE @searchTerm);";
            }
            else if (Session["currentRole"].ToString() == "cosec user")
            {
                query = "SELECT * FROM BoardResolution B LEFT JOIN Request R ON B.requestID = R.requestID WHERE B.type <> 'T' and R.assignedTo = @id and (B.type LIKE @searchTerm or B.title LIKE @searchTerm OR B.description LIKE @searchTerm OR CONVERT(VARCHAR, B.due_date, 23) LIKE @searchTerm OR CONVERT(VARCHAR, B.created_date, 23) LIKE @searchTerm OR B.requestID LIKE @searchTerm OR B.createdBy LIKE @searchTerm);";
            }
        ...
        protected void DdlStatus_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                if (roleName == "client user")
                {
                    ...
                    {
                        query = "Select * from BoardResolution B  LEFT JOIN [User] U ON B.createdBy  = U.userID LEFT JOIN [User] M ON M.userID = @id LEFT JOIN Request R ON B.requestID = R.requestID WHERE U.companyID = M.companyID AND B.type <> 'T' and R.status = @status";
                ...
                else if (roleName == "cosec user")
                {
                    ...
                    else
                    {
                        ...
                        query = "SELECT * FROM BoardResolution B LEFT JOIN Request R ON B.requestID = R.requestID WHERE B.type <> 'T' and R.assignedTo =@id and R.status = @status;";
                    }
                }
                else
                {
